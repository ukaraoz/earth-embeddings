...
pytest --cov=./earth_embeddings --cov-report=term-missing tests

# Generate HTML coverage report
pytest --cov=./earth_embeddings --cov-report=html tests
# Open htmlcov/index.html in your browser

# Show coverage for specific file
pytest --cov=./earth_embeddings/services/cache.py tests/services/test_cache.py
```
...
```python
@pytest.mark.asyncio
@patch('earth_embeddings.services.cache.get_cached')
...
async def test_with_custom_env():
    ...
    from earth_embeddings.settings import settings
...
def test_settings_load():
    ...
    assert settings.project_name == "earth_embeddings"
...
pytest --cov=./earth_embeddings --cov-fail-under=80 tests
```
